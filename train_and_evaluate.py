...
def train_step(module, criterion, optimizer, x, y, scheduler):
    module.train()
...
def train_and_evaluate(module, criterion, optimizer, train_loader, epochs, validation_loader, savefile_name,
                       device, use_last_module=False, scheduler=None):
    ...
    for epoch in range(epochs):
        ...
        for k, (batch_sequences, batch_labels) in enumerate(train_loader):
            ...
            module.train()
            loss = train_step(module, criterion, optimizer, batch_sequences, batch_labels, scheduler)
        ...
        print(f"macro: {macro_performance}")

    print("Performance on all epochs" + str(epoch_performance))

    return epoch_performance


def rnn_training(X, y, savefile_name, device, epochs=100):
    train_dataloader, val_dataloader = get_dataloaders(X, y)

    model = MyRNN(in_features=7, hidden_features=10, num_layers=2).to(device)
